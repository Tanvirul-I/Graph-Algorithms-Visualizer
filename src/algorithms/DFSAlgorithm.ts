...
export class DFSAlgorithm implements Algorithm {
        ...
        initialize(graph: Graph): void {
                ...
                this.stepInfo = `Initialized DFS with start node ${this.startNode.id}.`;
        }
        ...
        step(): boolean {
                if (this.stack.length === 0) {
                        ...
                        this.state.currentEdges = [];
                        this.stepInfo += `<br>DFS traversal completed. All reachable nodes have been visited.`;
                        return false;
                }
                ...
                for (let i = neighbors.length - 1; i >= 0; i--) {
                        const { node: neighbor, edge } = neighbors[i];
                        if (!this.visited.has(neighbor.id)) {
                                ...
                                this.stepInfo += `<br>Pushed node ${neighbor.id} via edge ${edge.source.id}, ${edge.target.id} with weight ${weightText} to stack.`;
                        }
                }
